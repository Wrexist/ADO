...
import type { ConnectionStatus } from '@ado/shared';
...
export async function fetchConnections(): Promise<ConnectionStatus[]> {
  ...
  return ((await res.json()) as { connections: ConnectionStatus[] }).connections;
}
...
export async function saveConnection(id: string, value: string): Promise<ConnectionStatus> {
  ...
  return ((await res.json()) as { status: ConnectionStatus }).status;
}
